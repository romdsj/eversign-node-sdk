 ...
 */
function Client(accessKey, businessId, sandbox = false){
            ...
            async.forEachOf(document.getFiles(), function (file, index, callback) {
              ...
              _.forEach(document.files, function(file, index) {
                var sendFile = _.mapKeys(file, function(value, key) {
                  // Lodash change the way they handle numbers in snakeCase method
                  return _.snakeCase(key).replace('base_64', 'base64');
                });
   ...
   * @return {Promise}                     [description]
   */
  this.downloadFinalDocumentToPath = function (document, auditTrail = false) {
    if(!document.getIsCompleted())
      throw new Error('To Download the final File the Document needs to be completed first');
    return downloadDocumentToPath(document, auditTrail);
  };
   ...
   * @return {Promise}          [description]
   */
  this.downloadRawDocumentToPath = function(document) {
    return downloadDocumentToPath(document, 0, config.DOCUMENT_RAW_URL);
  };

  function downloadDocumentToPath(document, auditTrail = 0, type = config.DOCUMENT_FINAL_URL) {
    return new Promise(function(resolve, reject) {
      getBusinessId().then(function (businessId) {
        if (!document) {
          throw new Error('To Download the Document you need to set the document');
        }
        var parameters = {
          "business_id" : businessId,
          "document_hash" : document.getDocumentHash(),
        ...
        var request = new ApiRequest("GET", accessKey, type, "Document", parameters);
